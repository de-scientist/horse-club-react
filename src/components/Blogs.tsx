 ...
 */
const BlogCard: React.FC<BlogCardProps> = ({ title, synopsis, imageSrc, imageAlt, chips, meta, link = "#" }) => {
  return (
    <div className="blog-card">
      <div className="blog-card-img-wrapper">
        <img src={imageSrc} alt={imageAlt} className="blog-card-featured-image" />
      </div>
      <div className="blog-card-chips-container">
        {chips.map((chip, idx) => (
          <a key={idx} href="#" className="blog-card-chip">{chip}</a>
        ))}
      </div>
      <a href={link} className="blog-card-title">
        <h3 className="blog-card-title-title">{title}</h3>
      </a>
      <p className="blog-synopsis" dangerouslySetInnerHTML={{ __html: synopsis }} />
